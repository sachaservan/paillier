...
func (this *ThresholdKey) computeDecryption(cprime *big.Int) *big.Int {
	l := L(cprime, this.N)
	value := new(big.Int).Mod(new(big.Int).Mul(this.CombineSharesConstant(), l), this.N)
	return value
}
...
//  VerifyDecryption checks if the decryption of encryptedMessage is correct.
//  It verifies all the zero-knowledge proofs, the value of the encrypted
//  and decrypted message.
//  The method returns `nil` if verification succeeds.  Otherwise it returns an
//  error message.
func (this *ThresholdKey) VerifyDecryption(encryptedMessage *Ciphertext, decryptedMessage *big.Int, shares []*PartialDecryptionZKP) error {
	for _, share := range shares {
		if share.C.Cmp(encryptedMessage.C) != 0 {
...
//  Decrypt the ciphertext and returns the partial decryption
...
func (this *ThresholdPrivateKey) DecryptAndProduceZKP(c *big.Int) (*PartialDecryptionZKP, error) {
	...
	r, err := rand.Int(rand.Reader, this.GetNSquare())
	if err != nil {
		return nil, err
	}
	//  compute a
...
func (this *ThresholdPrivateKey) Verify(random io.Reader) error {
	...
	c := this.Encrypt(m)
	proof, err := this.DecryptAndProduceZKP(c.C)
	if err != nil {
		return err
	}
	if !proof.Verify() {
